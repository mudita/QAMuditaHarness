...
class FsInitGet(GenericTransaction):
    ...
    def __init__(self, path: str, filename: str):
        self.request = Request(
            Endpoint.FILESYSTEM, Method.GET, {"fileName": path + "/" + filename}
        )
...
class FsGetChunk(GenericTransaction):
    ...
    def __init__(self, id: int, chunkNo: int):
        self.request = Request(
            Endpoint.FILESYSTEM, Method.GET, {"rxID": id, "chunkNo": chunkNo}
        )
...
class FsRemoveFile(GenericTransaction):
    def __init__(self, filename: str) -> None:
        ...
        self.request = Request(
            Endpoint.FILESYSTEM, Method.DEL, {"removeFile": filename}
        )
...
class FsRenameFile(GenericTransaction):
    def __init__(self, filename: str, destFilename: str) -> None:
        ...
        self.request = Request(
            Endpoint.FILESYSTEM,
            Method.PUT,
            {"renameFile": filename, "destFilename": destFilename},
        )
...
def get_transfer(
    harness: Harness, logDir: str, fileName: str, rxID, fileSize, chunkSize
):
    """
...
class FsInitPut(GenericTransaction):
    ...
    def __init__(self, path: str, filename: str, size: int, crc32: str):
        self.request = Request(
            Endpoint.FILESYSTEM,
            Method.PUT,
            {"fileName": path + "/" + filename, "fileSize": size, "fileCrc32": crc32},
        )
...
class FsPutChunk(GenericTransaction):
    ...
    def __init__(self, txID: int, chunkNo: int, data: bytearray):
        self.request = Request(
            Endpoint.FILESYSTEM,
            Method.PUT,
            {"txID": txID, "chunkNo": chunkNo, "data": data},
        )
...
def put_file(harness: Harness, file: str, where: str, filename: str = None):
    ...
    with open(file, "rb") as l_file:
        with tqdm(
            ...
            unit="B",
            unit_scale=True,
            desc=f"{file} -> {where}/{filename}",
        ) as p_bar:
            for chunk in iter(partial(l_file.read, ret.chunkSize), b""):
                FsPutChunk(
                    ret.txID, chunkNo, base64.standard_b64encode(chunk).decode()
                ).run(harness)
                chunkNo += 1
