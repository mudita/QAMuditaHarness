...
class GetPhoneLockStatus(GenericTransaction):
    ...
    def __init__(self):
        self.request = Request(
            Endpoint.USBSECURITY, Method.GET, {"category": "phoneLockStatus"}
        )
...
class GetPhoneLockTime(GenericTransaction):
    ...
    def __init__(self):
        self.request = Request(
            Endpoint.USBSECURITY, Method.GET, {"category": "phoneLockTime"}
        )
...
class SetPhoneLockOff(GenericTransaction):
    ...
    def __init__(self, passcode: list = None):
        if passcode is None:
            passcode = [3, 3, 3, 3]
        self.request = Request(
            Endpoint.USBSECURITY, Method.PUT, {"phoneLockCode": passcode}
        )
